...
class Trivia(commands.Cog):
    ...
    async def trivia(self, ctx):
        ...
        # Récupérer les questions Trivia
        questions = await self.fetch_trivia_question()
        if not questions:
            await ctx.send("❌ Erreur : Impossible de récupérer les questions Trivia.")
            return

        # Choisir une question aléatoire et décoder les entités HTML
        question_data = random.choice(questions)
        question = html.unescape(question_data["question"])  # Décodage des entités HTML
        correct_answer = html.unescape(question_data["correct_answer"])  # Décodage des entités HTML
        options = [html.unescape(option) for option in
                   question_data["incorrect_answers"] + [correct_answer]]  # Décodage

        # Mélanger les options
        random.shuffle(options)

        # Stocker l'état de la question active pour l'utilisateur
        self.trivia_active[ctx.author.id] = {"answer": correct_answer.lower(), "options": options}

        # Créer l'embed pour afficher la question et les options
        embed = discord.Embed(
            title="🧠 Trivia Gaming 🧠",
        ...
        embed.set_thumbnail(
            url="https://cdn.pixabay.com/photo/2024/05/24/16/40/ai-generated-8785422_640.jpg")  # Placeholder
        embed.set_footer(
            text=f"Bonne chance, {ctx.author.name} ! Réponds avec `/answer <numéro>` dans les 30 secondes.",
            icon_url=ctx.author.avatar.url)
        await ctx.send(embed=embed)
        logger.info(f"🎮 {ctx.author} a lancé une question Trivia : {question}")

        # Timer de 30 secondes pour expirer la question
        def timeout_check():
            if ctx.author.id in self.trivia_active:
